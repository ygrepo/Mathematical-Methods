...
\item [b.]
One property of the Laplace transform is $L[t^n f(t)] = (-1)^n \frac{d^n \bar{f}(s)}{ds^n}$ for $n=1,2,3,\cdots$,
...
in the previous equation that $k<n$. By multiplying two polynomials of degree $M$ and $N$, the result is a polynomial of degree $M + N$.
...
\]
Going back to our initial equation of interest:
\ba
...
\ea
The first term is $0$ as it is a sum of terms $[x^{l+k} e^{-x}]_0^\infty = 0$ and after applying integration by parts $k$ times, we are left with
\[
