...
Now $X(x) = A \cos(\sqrt{k} x)$ and $X'(x) = - A \sqrt{k} \sin(\sqrt{k} x)$. Next, with $u_x(a,y) = 0$ gives $X'(a) Y(y) = 0   \rightarrow  X'(a) = 0$.
$X'(a) = 0$ therefore $- A \sqrt{k} \sin(\sqrt{k} a) = 0  \rightarrow  \sqrt{k} a = n \pi$. Therefore $k_n =  \frac{n^2 \pi^2} {a^2}$ and 
\[
	X_n(x) = A_n \sin(\frac{n \pi}{a} x )
\]
